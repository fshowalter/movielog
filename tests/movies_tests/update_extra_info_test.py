...
def detail_for_title_mock(mocker: MockFixture) -> Any:
    return mocker.patch(
        "movielog.movies.imdb_http.detail_for_title",
        return_value=(
...
def test_creates_new_extra_infos_for_ones_that_do_not_exist(
    tmp_path: str, detail_for_title_mock: MockFixture,
) -> None:
    with open(
        os.path.join(os.path.dirname(__file__), "update_extra_info_test_output.yml"),
        "r",
    ) as expected_content_file:
        expected_file_content = expected_content_file.read()

    movies.update_extra_info(["tt0092106"])

    detail_for_title_mock.assert_called_once_with("tt0092106")

    with open(
        os.path.join(tmp_path, "movies", "the-transformers-the-movie-1986.yml"), "r",
    ) as new_file:
        assert new_file.read() == expected_file_content


def test_does_not_call_imdb_for_extra_infos_that_exist(
    tmp_path: str, detail_for_title_mock: MockFixture,
) -> None:
    output_path = os.path.join(tmp_path, "movies")
