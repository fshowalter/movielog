...
from testtools.keys import Backspace, Down, End, Enter, Escape
from testtools.types import MockInput


@pytest.fixture(autouse=True)
def mock_create_viewing(mocker: MockerFixture) -> MagicMock:
    return mocker.patch("movielog.cli.add_viewing.movielog_api.create_viewing")


@pytest.fixture(autouse=True)
def mock_create_review(mocker: MockerFixture) -> MagicMock:
    return mocker.patch("movielog.cli.add_viewing.movielog_api.create_review")
...
def test_calls_add_viewing(
    mock_input: MockInput, mock_create_viewing: MagicMock
) -> None:
    mock_input(
        [
            "Rio Bravo",
    ...
    mock_create_viewing.assert_called_once_with(
        imdb_id="tt0053221",
        title="Rio Bravo",
        venue="Blu-ray",
...
def test_does_not_call_add_viewing_if_no_movie(
    ...
    mock_create_viewing.assert_not_called()
...
def test_does_not_call_add_viewing_if_no_grade(
    mock_input: MockInput, mock_create_viewing: MagicMock
) -> None:
    mock_input(
        [
