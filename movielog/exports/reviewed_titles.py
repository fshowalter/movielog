        ...
        for member_id, member_titles in indexed_names.items():
            ...
            sliced_titles = _slice_list(
                source_list=sorted(
                    ...
                    key=lambda title: (title.release_date, title.imdb_id),
                ),
                matcher=_build_imdb_id_matcher(review.imdb_id),
            )
            ...
            cast_and_crew[member_id] = _JsonMoreCastAndCrewMember(
                ...
                titles=[title.slug for title in sliced_titles if title.imdb_id != review.imdb_id],
            )
...
def export(repository_data: RepositoryData) -> None:
    ...
    exporter.serialize_dicts_to_folder(
        json_reviewed_titles,
        "reviewed-titles",
        filename_key=lambda name: name["review"],
    )
