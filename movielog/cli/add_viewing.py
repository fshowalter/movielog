...
from movie_db.cli import _ask, _radio_list, _select_movie
...
def _ask_for_movie() -> Optional[queries.MovieSearchResult]:
    search_result = _select_movie.prompt('Movie title: ')

    if search_result and confirm(_select_movie.format_search_result(search_result, extra_text='?')):
...
def _ask_for_date() -> Optional[date]:
    ...
    date_string = _ask.prompt('Date: ', extra_rprompt='YYYY-MM-DD format.', validator=validator)
    if not date_string:
        return None
...
def _ask_for_venue() -> Optional[str]:
    ...
    options = _radio_list.VenueOptions([])
    ...
    while selected_venue is None:
        ...
        selected_venue = _radio_list.prompt(
            title=HTML('Select venue:'),
            options=options,
        )
