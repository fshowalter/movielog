...
def _update_json_title_with_title_page_data(
    ...
    title_page = imdb_http_title.get_title_page(token, json_title["imdbId"])
...
def update_from_imdb_pages(token: str) -> None:
    ...
    with (
        ...
        for index, json_title in enumerate(titles):
            ...
            _update_json_title_with_title_page_data(token, updated_title, watchlist_performer_ids)
...
def update_title(
    token: str, json_title: json_titles.JsonTitle, watchlist_performer_ids: set[str]
) -> None:
    if json_title["imdbId"] in FrozenTitles:
        return

    updated_json_title = deepcopy(json_title)
    _update_json_title_with_title_page_data(token, updated_json_title, watchlist_performer_ids)
